...
| Launch | Hands off to `openclaw onboard` for hook selection + interactive hatching |

All steps detect existing config and skip what's already done. Re-run safely at any time. At the end, the wizard hands off to `openclaw onboard` for the hatching process. Onboard skips already-configured steps and proceeds to hook selection and hatching. During the hatch, the bot says "Wake up, my friend..." and asks you questions to build its personality (SOUL.md) interactively. Do not pre-write SOUL.md — the hatching process generates it.
...
├── openclaw-reset.sh        # Reset agent (wipe memory/sessions/personality, re-hatch)
├── templates/
...
This wipes memory (vector store + markdown logs), sessions (conversation history), and workspace identity files (SOUL.md, AGENTS.md, IDENTITY.md, etc.), then launches `openclaw onboard` to re-hatch the bot from scratch. All API keys, provider credentials, gateway tokens, and config settings are preserved. The hatching process interactively rebuilds the bot's personality.
